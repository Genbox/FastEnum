...
public class DenseLookupTests
{
    ...
    private static bool CompileAndRun(string source)
    {
        ...
        driver.RunGeneratorsAndUpdateCompilation(compilation, out Compilation output, out var diagnostics, token);
        Assert.Empty(diagnostics);

        using MemoryStream stream = new MemoryStream();
        var emitted = output.Emit(stream, cancellationToken: token);
