...
public class TransformHelperTests
{
    ...
    public void PresetTest(string input, string expected, EnumTransform? preset) => Assert.Equal(expected, TransformHelper.TransformName(input, preset ?? EnumTransform.None, null, null));

    [Theory]
    [InlineData("HelloWorld", "HennoWorld", "/ll/nn/")]
    [InlineData("HelloWorld", "HiWorld", "/^...../Hi/")]
    public void RegexTest(string input, string expected, string? regex) => Assert.Equal(expected, TransformHelper.TransformName(input, EnumTransform.None, regex, null));
    ...
    public void TransformHelperTest(string input, string expected, string? casePattern) => Assert.Equal(expected, TransformHelper.TransformName(input, EnumTransform.None, null, casePattern));
}