...
public class EnumGenerator : IIncrementalGenerator
{
        ...
        context.RegisterSourceOutput(sp, (spc, specs) =>
        {
            ...
            foreach (EnumSpec enumSpec in specs)
            {
                try
                {
                    ...
                    spc.AddSource($"{enumSpec.FullName}_Enums.g.cs", GetSource(header, enumSpec, spec => EnumClassCode.Generate(spec, wrapperPublic)));
                    spc.AddSource($"{enumSpec.FullName}_Extensions.g.cs", GetSource(header, enumSpec, EnumExtensionCode.Generate));
                }
                catch (Exception e)
                {
    ...
    {
#if RELEASE
        AssemblyName assemblyName = typeof(EnumGenerator).Assembly.GetName();
        return $$"""
                 // <auto-generated />
                 // Generated by {{assemblyName.Name}} {{assemblyName.Version}}
                 // Generated on: {{DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss", DateTimeFormatInfo.InvariantInfo)}} UTC
                 #nullable enable
                 """;
#else
        return """
